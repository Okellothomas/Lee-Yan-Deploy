...
interface IParams {
  tourId?: string;
  tourParams: any;
  category?: string; // Make county optional
}
...
export default function Stay({ tourParams }: IParams) {
  ...
  const normalizedCounty = category ? category.toLowerCase() : ""; // Normalize county safely
  const listingsPremium = tours.filter(listing => listing.category && listing.category.toLowerCase() === normalizedCounty);
    ...
    <div>
      <div className="alldestinations-main flex flex-col items-center justify-center text-lg font-bold">
        <h1 className="alldestinations-white-main">
          <span className="text-white">Choose an exclusive {category || "Stays"}</span>
        </h1>
      </div>
      <Container>
