...
interface ListingCardProps {
    data: safePropertyReservation;
    reservation?: safePropertyReservation;
...
const PropertyClientCard: React.FC<ListingCardProps> = ({
    data,
    reservation,
    onAction,
    disabled,
...
function formatDate(dateString: any) {
  ...
  return (
      <div
        onClick={() => router.push(`/property/${data?.Property.id || ""}`)} // Handle null data or id
          ...
          <div className="flex flex-col gap-2 w-full main-image-small-screen border-[1px] border-solid border-neutral-300 pb-1 rounded-xl">
              <div className="aspect-square w-full h-[25vh] relative overflow-hidden rounded-t-xl">
                  <Image
                      fill
                      alt="Listing"
                      src={data?.Property.imageSrc[0] || ""} // Handle null data or imageSrc sure one 
              ...
              <div className="font-light text-neutral-500 text-sm">
                  <span className="text-neutral-800">Balance:</span> Ksh. { data.Property.price  -  data.paymentDetails.Body.stkCallback.CallbackMetadata.Item[0].Value}
              </div> 
              </div>
              <hr />
              <div className="flex justify-between mx-2 items-center">
              <div className="font-light text-neutral-500 text-sm">
                 <span className="text-neutral-800">Agent:</span>: {data.Property.user.name}
              </div>
              <div className="font-light text-neutral-500 text-sm">
                  <span className="text-neutral-800">Agent Contact:</span> {data.Property.user.contact}
              </div> 
              </div>
              <hr />
              <div className="flex justify-between mx-2 items-center">
              <div className="font-light text-neutral-500 text-sm">
                 <span className="text-neutral-800">Agent email:</span> {data.Property.user.email}
              </div>
              {/* <div className="font-light text-neutral-500 text-sm">
